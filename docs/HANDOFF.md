...
Дата: 2026-09-20. Версия кода: `7.5.0`, UI-сборка **b56**, тестов: **198**.
...
## 6.4 Копия SSD (macOS → сервер) — как проверять и копировать
- ПРОВЕРКА: `bash scripts/ssd_copy_verify.sh "/Volumes/SSD" root@192.168.100.40 /mnt/video/ssd_backup`
  (наличие + размеры всех файлов; учитывает Unicode NFC/NFD и пробелы по краям).
- Побайтово (долго): `rsync -a -n -c --out-format="%i %n" "/Volumes/SSD/" root@...:/mnt/video/ssd_backup/`
- ВАЖНО: macOS пишет имена в NFD, Linux — NFC. Для будущих догрузок использовать
  rsync 3.x с `--iconv=UTF-8-MAC,UTF-8` (системный macOS rsync этого не умеет),
  иначе появятся папки-двойники с другой формой имени. Сервер уже нормализован в NFC.
