...
from .postiz import PostizRouter
...
def build(args: argparse.Namespace) -> dict:
    ...
    # Telegram в режиме send_via=bot отправляем сами (Bot API: превью ролика над текстом).
    # Роутер разводит такие посты (`tg:<id>`) и посты Postiz, поэтому панель,
    # синхронизация статусов и удаление работают с одним объектом как раньше.
    tg_pub = create_telegram_publisher(cfg)
    if tg_pub is not None:
        logger.info("Telegram: публикация через Bot API, канал %s", cfg.platforms["telegram"].publish_chat_id)
        postiz = PostizRouter(postiz, tg_pub)
