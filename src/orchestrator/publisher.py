...
class Publisher:
    ...
    def publish(
        ...
        # B: link-режим — файл не отправляем ни при каком пути вызова (остаток серии,
        # тематические шортсы, правка очереди из панели). Канал должен получить ссылку
        # (Bot API: TelegramPublisher.send_post), а не файл: Bot API не примет >50 МБ.
        if media_path and pcfg is not None and \
                str(getattr(pcfg, "post_mode", "media") or "media").lower() == "link":
            logger.warning("link-режим %s: файл %s не отправляем, публикуем ссылку",
                           platform, media_path)
            try:
                self.db.log(entity_type, entity_id, platform, "link_mode_media_skipped",
                            str(media_path))
            except Exception:
        ...
        if post is None:
            ...
            # pid обнуляем: иначе строка 'error' с непустым postiz_post_id не резервируется
            # и не восстанавливается синхронизацией — пара не опубликуется уже никогда
            self.db.execute(
                "UPDATE entity_platform_status SET status='error', last_error=?, "
                "postiz_post_id=NULL "
                "WHERE entity_type=? AND entity_id=? AND platform=?",
        ...
        try:
            ...
            # строка не сохранилась, а пост уже создан — убираем пост, иначе он «сирота»,
            # а строка навсегда застрянет в publishing без postiz_post_id
            logger.exception("Не сохранил строку после создания поста %s — удаляю пост",
                             post.id)
            try:
                self.postiz.delete_post(str(post.id))
            except Exception:
                logger.warning("не удалил пост %s после сбоя сохранения", post.id)
            try:
                self._release_reserve(entity_type, entity_id, platform)
            except Exception:
                logger.debug("reserve release failed", exc_info=True)
            raise
        if scheduled_for:
