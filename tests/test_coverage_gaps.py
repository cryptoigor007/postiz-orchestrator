...
def test_tg_bot_ack_and_help(tmp_path):
    ...
    ack = bot.handle_update(own, "привет")
    # у ботов нет статуса «прочитано»: автоответ честно говорит, что это автоответ,
    # а признак чтения — 👍 на сообщении, которое я прочитал и на которое ответил
    assert "Автоответ" in ack and "👍" in ack and "Принято" not in ack
