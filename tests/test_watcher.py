...
def test_long_cover_detected(tmp_path):
    ...
    # обложка определяется по размерам картинки: горизонтальная (16:9) для длинного видео
    _png(series / "cover_16x9.png", 1920, 1080)
    w = Watcher(db, cfg, clock, [str(series.parent)])
    for _ in range(4):
        w.scan()
    lv = db.fetchone("SELECT * FROM long_videos")
    assert lv["cover_path"] and lv["cover_path"].endswith("cover_16x9.png")
...
# ---------- стандарт видео мейкера: папка серии + пакеты платформ ----------


def _png(path: Path, w: int, h: int) -> None:
    """Минимальный PNG-заголовок с заданными размерами (проверка обложек)."""
    path.write_bytes(
...
def _pack_series(root: Path) -> Path:
    """Серия по новому стандарту: info_metadata.txt + пакет youtube/."""
    series = root / "7"
    (series / "youtube" / "vertical").mkdir(parents=True)
    (series / "youtube" / "wide").mkdir(parents=True)
    short = series / "youtube" / "shorts" / "short_001"
    short.mkdir(parents=True)
    (series / "info_metadata.txt").write_text(
        "series: 7\npackage_title: ЗАГОЛОВОК ПАКЕТА\npackage_hashtags: #tag1 #tag2\n",
        encoding="utf-8",
    )
    (series / "youtube" / "vertical" / "master_9x16.mp4").write_bytes(b"m" * 400)
    (series / "youtube" / "vertical" / "final_9x16.mp4").write_bytes(b"v" * 400)
    (series / "youtube" / "wide" / "master_16x9.mp4").write_bytes(b"m" * 400)
    (series / "youtube" / "wide" / "final_16x9.mp4").write_bytes(b"w" * 400)
    (series / "youtube" / "wide" / "7_description.txt").write_text(
        "Описание из пакета", encoding="utf-8")
    (short / "short_001.mp4").write_bytes(b"s" * 400)
    (short / "short_001_title.txt").write_text("Заголовок шортса", encoding="utf-8")
    (short / "short_001_cover.jpg").write_bytes(b"jpg")
    (short / "._short_001_cover.jpg").write_bytes(b"junk")
    return series


def _platform_env(tmp_path, pack=True):
    db = Database(tmp_path / "pkg.sqlite")
    cfg = load_config(Path(__file__).resolve().parents[1] / "config.yaml")
    clock = FakeClock(datetime(2026, 3, 10, 12, 0, tzinfo=UTC))
    root = tmp_path / "content"
    series = _pack_series(root) if pack else None
    return db, cfg, clock, root, series


def test_platform_package_long_and_shorts(tmp_path):
    """Пакет платформы: только final своего формата, вертикаль youtube/ не берём никому."""
    db, cfg, clock, root, series = _platform_env(tmp_path)
    w = Watcher(db, cfg, clock, [str(root)])
    for _ in range(4):
        w.scan()
    lv = db.fetchone("SELECT * FROM long_videos")
    assert lv is not None
    assert lv["wide_path"].endswith("youtube/wide/final_16x9.mp4")
    # вертикаль внутри youtube/ не достаётся никому
    assert lv["vertical_path"] is None
    assert "final_9x16" not in json.dumps(lv)
    assert "master" not in json.dumps(lv)
    pm = json.loads(lv["platform_paths"])
    assert list(pm.keys()) == ["youtube"]
    assert pm["youtube"].endswith("youtube/wide/final_16x9.mp4")
    assert lv["title_text"] == "ЗАГОЛОВОК ПАКЕТА"
    assert lv["description_text"] == "Описание из пакета"
    assert lv["hashtags_text"] == "#tag1 #tag2"

    short = db.fetchone("SELECT * FROM shorts")
    assert short is not None
    assert json.loads(short["platform_paths"]) == {
        "youtube": str(series / "youtube" / "shorts" / "short_001" / "short_001.mp4")
    }
    assert short["parent_video_id"] == lv["id"]
    assert short["title_text"] == "Заголовок шортса"
    assert short["cover_path"].endswith("short_001_cover.jpg")
    assert "._" not in short["cover_path"]


def test_series_cover_by_size_for_wide_platform(tmp_path):
    """Обложки различаем по размерам: YouTube (16:9) берёт горизонтальную."""
    db, cfg, clock, root, series = _platform_env(tmp_path)
    _png(series / "обложка_телефон.png", 1080, 1920)
    _png(series / "covers_horiz.png", 1920, 1080)
    (series / "._covers_broken.png").write_bytes(b"\x89PNG\r\n\x1a\n" + b"\x00" * 4)
    w = Watcher(db, cfg, clock, [str(root)])
    for _ in range(4):
        w.scan()
    lv = db.fetchone("SELECT * FROM long_videos")
    assert lv["cover_path"].endswith("covers_horiz.png")


def test_vertical_only_cover_not_used_for_wide_platform(tmp_path):
    """Если есть только вертикальная обложка, горизонтальной платформе её не подставляем."""
    db, cfg, clock, root, series = _platform_env(tmp_path)
    _png(series / "cover_vertical.png", 1080, 1920)
    w = Watcher(db, cfg, clock, [str(root)])
    for _ in range(4):
        w.scan()
    lv = db.fetchone("SELECT * FROM long_videos")
    assert lv["cover_path"] is None


def test_legacy_layout_without_packages(env):
    """Старый формат (wide/vertical/shorts в корне серии) продолжает работать."""
    db, cfg, clock, root, series = env
    w = Watcher(db, cfg, clock, [str(root)])
    for _ in range(4):
        w.scan()
    lv = db.fetchone("SELECT * FROM long_videos")
    assert lv["wide_path"].endswith("wide/final_16x9.mp4")
    assert lv["vertical_path"].endswith("vertical/final_9x16.mp4")
    assert lv["platform_paths"] in (None, "")
    assert db.fetchone("SELECT COUNT(*) AS c FROM shorts")["c"] == 1


def test_package_without_final_is_skipped_and_reported(tmp_path):
    """Пакет без final своего формата: серию не берём, но скан сообщает о пропуске."""
    db = Database(tmp_path / "nf.sqlite")
    cfg = load_config(Path(__file__).resolve().parents[1] / "config.yaml")
    clock = FakeClock(datetime(2026, 3, 10, 12, 0, tzinfo=UTC))
    root = tmp_path / "content"
    series = root / "8"
    (series / "youtube" / "wide").mkdir(parents=True)
    (series / "youtube" / "wide" / "master_16x9.mp4").write_bytes(b"m" * 400)
    (series / "info_metadata.txt").write_text("series: 8", encoding="utf-8")
    w = Watcher(db, cfg, clock, [str(root)])
    stats = {}
    for _ in range(4):
        stats = w.scan()
    assert db.fetchone("SELECT COUNT(*) AS c FROM long_videos")["c"] == 0
    assert stats.get("no_final", 0) >= 1


def test_standalone_shorts_keep_folder_order(tmp_path):
    """Жалоба 2026-09-23: самостоятельные шортсы добавлялись в порядке скана (ш1, ш10, ш2…)."""
    db = Database(tmp_path / "o.sqlite")
    cfg = load_config(Path(__file__).resolve().parents[1] / "config.yaml")
    clock = FakeClock(datetime(2026, 3, 10, 12, 0, tzinfo=UTC))
    root = tmp_path / "ssd" / "ютюб" / "шортс"
    root.mkdir(parents=True)
    # реальный вид папки: шN <имя>/*_final.mp4 + *_titles.txt
    for name in (
        "ш10почему добро не всегда возвращается",
        "ш1 почему люди завидуют выложил",
        "Ш 19 почему деньги меняют людей",
        "ш2 почему мы откладываем жизнь",
    ):
        d = root / name
        d.mkdir()
        prefix = "clip"
        (d / f"{prefix}_final.mp4").write_bytes(b"clip" * 40)
        (d / f"{prefix}_titles.txt").write_text(f"Заголовок: {name}", encoding="utf-8")
    db.set_setting("watch_roots", json.dumps([{"path": str(root), "kind": "shorts"}]))
    w = Watcher(db, cfg, clock, [])
    for _ in range(4):
        w.scan()
    rows = db.fetchall("SELECT order_index, title_text FROM shorts ORDER BY order_index")
    assert [r["order_index"] for r in rows] == [0, 1, 2, 3]
    assert rows[0]["title_text"].startswith("ш1 ")
    assert rows[1]["title_text"].startswith("ш2 ")
    assert rows[2]["title_text"].startswith("ш10")
    assert rows[3]["title_text"].startswith("Ш 19")


def test_shorts_dir_order_skips_non_short_entries(env):
    """order_index шортсов серии считается только по папкам-шортсам (файлы/мусор не сдвигают номер)."""
    db, cfg, clock, root, series = env
    (series / "shorts" / "readme.txt").write_text("noise")
    (series / "shorts" / "short_002").mkdir()
    (series / "shorts" / "short_002" / "clip.mp4").write_bytes(b"short" * 50)
    (series / "shorts" / "short_010").mkdir()
    (series / "shorts" / "short_010" / "clip.mp4").write_bytes(b"short" * 50)
    w = Watcher(db, cfg, clock, [str(root)])
    for _ in range(4):
        w.scan()
    rows = db.fetchall("SELECT order_index, folder_path FROM shorts")
    idx = {r["folder_path"].rstrip("/").split("/")[-1]: r["order_index"] for r in rows}
    assert idx["short_001"] == 0
    assert idx["short_002"] == 1
    assert idx["short_010"] == 2


def test_scan_counts_only_new_standalone(tmp_path):
    """«Новых» в отчёте = реально добавленные: повторный скан не удваивает счётчик."""
    db = Database(tmp_path / "c.sqlite")
    cfg = load_config(Path(__file__).resolve().parents[1] / "config.yaml")
    clock = FakeClock(datetime(2026, 3, 10, 12, 0, tzinfo=UTC))
    root = tmp_path / "shorts"
    root.mkdir()
    for name in ("ш1 первая", "ш2 вторая", "ш10 десятая"):
        d = root / name
        d.mkdir()
        (d / "clip_final.mp4").write_bytes(b"clip" * 40)
        (d / "clip_titles.txt").write_text(f"Заголовок: {name}", encoding="utf-8")
    db.set_setting("watch_roots", json.dumps([{"path": str(root), "kind": "shorts"}]))
    w = Watcher(db, cfg, clock, [])
    first = w.scan()  # файлы ещё не «устоялись»: нашли 3, но никого не добавили
    assert first["standalone"] == 0 and first["standalone_found"] == 3
    second = w.scan()  # устоялись — добавили 3
    assert second["standalone"] == 3 and second["standalone_found"] == 3
    third = w.scan()  # повторный скан ничего не добавляет и не удваивает счётчик
    assert third["standalone"] == 0, "повторный скан ничего не добавляет"
    assert third["standalone_found"] == 3, "найдено в папках — по-прежнему 3"
