...
def test_composed_index_inlines_assets_and_key(env):
    ...
    try:
        code, body, ctype = api.handle("GET", "/webapp/k/s3cret/b/8/", {}, b"")
        assert code == 200
        assert ctype.startswith("text/html")
        assert b'src="app.js' not in body
        assert b"__WEBAPP_KEY__" in body
        assert b'"s3cret"' in body
        assert b"[build b8]" in body
        code, body, _ = api.handle("GET", "/webapp/b/8/", {}, b"")
        assert b'__WEBAPP_KEY__=""' in body
    finally:
...
def test_build_path_serves_app_and_assets(env):
    ...
    assert code == 200
    assert b"Orchestrator" in body
    code, body, ctype = api.handle("GET", "/webapp/b/8/app.js", {}, b"")
    assert code == 200
