(() => {
  ...
  const I18N = {
    ru: {
      ...
      scan_in_base: "В базе по этим папкам", scan_restore_btn: "Вернуть удалённое и запустить скан", t_restored: "Возвращено",
      ...
      scan_no_final: "Пакеты платформ без final (не публикуются)",
      ...
      nav_status: "Обзор", nav_folders: "Видео", nav_calendar: "Календарь", nav_queue: "Очередь",
      ...
      t_distributed: "Распределено", t_link_saved: "Ссылка сохранена", t_scan: "Скан",
      ...
      st_waiting_yt: "ждёт публикацию на YouTube",
    ...
    en: {
      ...
      scan_in_base: "In base for these folders", scan_restore_btn: "Restore removed and start", t_restored: "Restored",
      ...
      scan_no_final: "Platform packages without final (not published)",
      ...
      nav_status: "Home", nav_folders: "Media", nav_calendar: "Calendar", nav_queue: "Queue",
      ...
      t_distributed: "Distributed", t_link_saved: "Link saved", t_scan: "Scan",
      ...
      st_waiting_yt: "waiting for YouTube",
  ...
  const state = {
    ...
    queueSelected: {},
    queueBulkTags: null,
    // Проекты (ниши): выбранный проект фильтрует очередь, календарь и ошибки
    project: (() => { try { return localStorage.getItem("project") || ""; } catch (_) { return ""; } })(),
    projects: null,
  };
  ...
  function openCoverPicker(opts) {
    ...
    function _escHandler(ev) { if (ev.key === "Escape") close(); }
  ...
  function statusBadge(status, waiting, kind) {
    ...
    if (waiting) {
      // у отдельного шортса премьеры нет — он ждёт свою публикацию на YouTube
      const label = kind === "short" ? t("st_waiting_yt") : t("st_waiting");
      out += ` <span class="badge warn" title="${esc(t("waiting_hint"))}">${label}</span>`;
    }
    return out;
  }
  ...
  async function load() {
    ...
    try {
      let data;
      if (!state.projects) {
        try { state.projects = await api("/projects"); } catch (_) { state.projects = { items: [] }; }
      }
  ...
  function renderFolders(d) {
    ...
    const scanPanel = sc ? `<div class="panel"><div class="panel-header">${t("scan")}</div>
      ...
      ${(sc.no_final || 0) > 0 ? `<div class="row"><span class="meta warn-text">${t("scan_no_final")}: ${sc.no_final}</span></div>` : ""}
      ...
      <div class="row"><span class="meta">${t("scan_found")}: ${t("scan_films")} ${st.long || 0} · ${t("scan_shorts")} ${st.shorts || 0} · ${t("scan_standalone")} ${(st.standalone_found != null ? st.standalone_found : st.standalone) || 0}</span></div>
      <div class="row"><span class="meta">${t("scan_in_base")}: ${t("scan_films")} ${tot.long || 0} · ${t("scan_shorts")} ${tot.shorts || 0}</span></div>
      ...
      <div class="form-row">
        ...
        <button class="btn secondary" data-act="scan-restore">${t("scan_restore_btn")}</button>
      </div></div>` : "";
  ...
  // Подпись периода: «23 сентября» / «22–28 сентября» / «Сентябрь 2026»
  const dayMonth = (iso) => {
    try {
      const [y, m, d] = iso.split("-").map(Number);
      return new Intl.DateTimeFormat(state.lang === "ru" ? "ru-RU" : "en-US",
        { day: "numeric", month: "long" }).format(new Date(y, m - 1, d, 12));
    } catch (_) { return iso; }
  };
  const monthYear = (iso) => {
    try {
      const [y, m] = iso.split("-").map(Number);
      const s = new Intl.DateTimeFormat(state.lang === "ru" ? "ru-RU" : "en-US",
        { month: "long", year: "numeric" }).format(new Date(y, m - 1, 1, 12));
  ...
  function calRow(g) {
    ...
    // пара «платформа → статус» одной строкой, плашки прижаты к правому краю карточки
    // (правые края пар совпадают между событиями — см. .cal-row .cal-badges)
    return `<div class="item cal-row">
      <span class="mono cal-time">${esc(g.time || "—")}</span>
      <div class="item__main">
        <div class="item__title">${esc(g.title)}</div>
        <div class="item__meta">${icons}${link ? `<span class="cal-link">${link}</span>` : ""}</div>
        <div class="item__meta cal-badges">${badges}</div>
      </div>
    </div>`;
  }

  // Шаг периода — один код на стрелки ‹ › и на свайп (обработчик cal-shift уже умел
  // и неделю, и месяц: раньше их просто нечем было вызвать).
  function shiftCal(step) {
  ...
  function renderCalendar(d) {
    const days = (d.days || [])
      .map((x) => { const items = filterByProject(x.items); return { ...x, items, count: items.length }; })
    ...
    const period = (() => {
      ...
      if (view === "week") {
        ...
        const text = start.slice(0, 7) === end.slice(0, 7)
          ? `${Number(start.slice(8))}–${dayMonth(end)}`
    ...
    // Строка навигации одна на все три вида: стрелки 44×44 у краёв, период по центру.
    // P4: внутри — крупные SVG-шевроны (icon 26px), а не текстовые глифы ‹ ›.
    const calNav = (label) => `<div class="cal-nav">
      <button class="btn secondary cal-step" data-act="cal-shift" data-n="-1" aria-label="${esc(t("cal_prev"))}">${icon("chevL", 22)}</button>
      <b class="cal-period">${label}</b>
      <button class="btn secondary cal-step" data-act="cal-shift" data-n="1" aria-label="${esc(t("cal_next"))}">${icon("chevR", 22)}</button></div>`;
    const bar = `<div class="panel"><div class="cal-bar">${seg([
      ["day", t("cal_day")], ["week", t("cal_week")], ["month", t("cal_month")]])}`
      + `${pch ? `<div class="chips">${pch}</div>` : ""}`
      + `<button class="btn secondary sm" data-act="cal-today">${t("cal_today")}</button></div></div>`;
    if (!days.length) {
    ...
    if (view === "day") {
      ...
      body = `<div class="panel">${calNav(period)}
        ${groups.map(calRow).join("") || `<div class="empty">${t("calendar_empty")}</div>`}</div>`;
    } else if (view === "week") {
      const start = addDaysIso(cur, -((weekdayIso(cur) + 6) % 7)); // понедельник
      ...
      for (let dd = 1; dd <= daysInMonth; dd++) {
        ...
        const day = byDate.get(ds);
        const groups = day ? calPostGroups(day.items) : [];
        // S1: в ячейке месяца ~30px под содержимое — больше двух иконок не влезает,
        // иначе они «просыпаются» на соседние дни. Остальные платформы — в дневном виде.
        // P4: строк иконок не больше трёх — иначе строка сетки выше соседних и «Месяц»
        // раздувается от одного плотного дня; остальные посты — счётчиком рядом с числом.
        const shown = groups.slice(0, 3);
        const dots = shown.map((g) => {
          const ico = g.items.slice(0, 2).map((it) => pIcon(it.platform)).join("");
          return `<span class="cal-dot">${ico}</span>`;
        }).join("");
        const rest = groups.length - shown.length;
        cells.push(`<button class="cal-cell${day ? " has" : ""}${ds === todayIso ? " today" : ""}" data-act="cal-day" data-d="${ds}">
          <span class="cal-d">${dd}${rest > 0 ? `<span class="cal-cnt">+${rest}</span>` : ""}</span>${dots}</button>`);
      }
    ...
    // .cal-swipe — поверхность жеста: горизонтальный свайп перелистывает период,
    // вертикальная промотка остаётся браузеру (touch-action: pan-y в CSS).
    content().innerHTML = `<div class="view-enter">${bar}<div class="cal-swipe">${body}</div></div>`;
  }
  ...
  async function bulkDelete() {
    ...
    for (const it of uniq) {
      ...
      try {
        ...
        if (r && (r.cascade || []).includes("telegram")) deps.push(it);
      } catch (e) {
        failed++;
    ...
    unbusy();
    state.queueSelected = {};
    const base = cascaded ? `${t("queue_removed")}: ${uniq.length} · ${t("cascade_deleted")}`
      : `${t("queue_removed")}: ${uniq.length}`;
  ...
  // ===== проекты (ниши): фильтр очереди/календаря/ошибок =====
  function projectList() {
    return ((state.projects && state.projects.items) || []);
  }

  function inProject(it) {
    const p = state.project || "";
    if (!p) return true;
    return String((it && it.project) || "") === p;
  }

  function filterByProject(items) {
    return (items || []).filter(inProject);
  }

  function projChips(act) {
    const list = projectList();
    if (list.length < 2) return "";
    const cur = state.project || "";
    const one = (id, label) => `<button class="chip${cur === id ? " on" : ""}" data-act="${act}" `
      + `data-p="${esc(id)}" aria-pressed="${cur === id}">${esc(label)}</button>`;
    return [one("", t("proj_all"))]
      .concat(list.map((p) => one(p.id, p.title || p.id)))
      .join("");
  }

  function renderQueue(d) {
    const edit = state.queueEdit;
    const all = filterByProject(d.items || []);
    ...
    const rowHtml = (it) => {
      ...
      const form = (edit && edit.key === key)
        ? `<div class="q-edit-panel">
             ...
             <div class="btn-grid">
               <label class="field"><span class="field__label">${t("edit_date")}</span><input id="qe-date" type="date" value="${it.date || ""}"/></label>
               <label class="field"><span class="field__label">${t("edit_time")}</span><input id="qe-time" type="time" value="${it.time || ""}"/></label>
             </div>
             <div class="btn-grid">
      ...
      return `<div class="item q-item${checked ? " picked" : ""}${select ? " with-check" : ""}">
        ...
        <div class="item__main">
          ...
          <div class="item__meta">${statusBadge(it.status, it.waiting, it.entity_type)}</div>
        </div>
        ${col}
        ${form}</div>`;
    };
  ...
  function failedHtml(d) {
    const items = filterByProject((d && d.items) || []);
    if (!items.length) {
  ...
  function renderManual(d) {
    ...
    const scanBtns = platforms
      .map((p) => `<button class="btn secondary" data-act="manual-scan" data-p="${esc(p)}">${t("mu_scan")} · ${esc(p)}</button>`)
      .join(" ");
    ...
    const rows = items.map((it) => {
      ...
      if (it.match_status === "suggested" && (it.candidates || []).length) {
        ...
        actions = `<div class="row"><span class="pill ok">${t("mu_confirmed")}</span>
          <span class="meta">${esc(String(it.matched_entity_type || ""))}#${esc(String(it.matched_entity_id || ""))}</span></div>`;
      ...
      return `<div class="panel">
        ...
        <div class="row"><div class="title">${esc(it.platform)} · ${esc(it.published_at || "")}</div>
  ...
  function helpHtml() {
    return [
      ...
      helpSection("help_common", [
        ...
        ["st_published", "help_st_published"], ["st_scheduled", "help_st_scheduled"],
        ["st_failed", "help_st_failed"], ["st_paused", "help_st_paused"],
      ]),
    ].join("");
  }

  // P5: чипы одной строкой упираются в край скроллера — обрезанный третий чип читается
  // как дефект вёрстки. Маска (styles.css .chips.more-right/.more-left) включается только
  // когда список реально листается (scrollWidth > clientWidth), поэтому на широком экране
  // подсказки нет вовсе. Пересчёт — на прокрутку, ресайз и любую перерисовку (наблюдатель
  // с троттлингом через requestAnimationFrame, а не вызовы в двух десятках мест рендера).
  function syncChipFade() {
    document.querySelectorAll(".chips").forEach((sc) => {
      const over = sc.scrollWidth - sc.clientWidth;
      sc.classList.toggle("more-right", over > 1 && sc.scrollLeft < over - 1);
      sc.classList.toggle("more-left", over > 1 && sc.scrollLeft > 1);
    });
  }

  let _fadeRaf = 0;
  function queueChipFade() {
    if (_fadeRaf) return;
    _fadeRaf = requestAnimationFrame(() => { _fadeRaf = 0; syncChipFade(); });
  }
  ...
  async function onAction(act, el) {
    try {
      ...
      if (act === "cal-shift") {
        shiftCal(Number(el.dataset.n) || 1);
        return;
      }
      if (act === "cal-today") {
      ...
      if (act === "sched-save") {
        ...
        const settings = Object.assign({}, state.data?.sched?.settings || {});
        settings[key] = block;
      ...
      if (act === "group-add") {
        ...
        const groups = (state.data?.sched?.groups || []).concat([{ name, platforms: plats }]);
        await api("/groups", { method: "POST", body: JSON.stringify({ groups }) });
        toast(t("t_group_added"));
        return load();
      }
      if (act === "group-remove") {
        const groups = (state.data?.sched?.groups || []).filter((g) => g.name !== el.dataset.p);
        await api("/groups", { method: "POST", body: JSON.stringify({ groups }) });
      ...
      if (act === "queue-project" || act === "cal-project") {
        state.project = el.dataset.p || "";
        try { localStorage.setItem("project", state.project); } catch (_) {}
        return render();
      }
      if (act === "queue-filter") {
  ...
  function bind() {
    ...
    content().addEventListener("click", (e) => {
      // свайп по календарю не должен «проваливаться» в тап по дню (см. bindCalSwipe)
      if (state.calSwipeAt && Date.now() - state.calSwipeAt < 400
          && e.target.closest && e.target.closest(".cal-swipe")) return;
    ...
    bindCalSwipe(content());
  }

  // Свайп по календарю влево/вправо = ‹ › без дотягивания до кнопок.
  // Вертикальную промотку оставляем браузеру (touch-action: pan-y), порог и скорость — из Emil:
  // осознанный свайп от 60px ИЛИ быстрый «щелчок» (>=24px и >=0.11 px/ms). Мышь не трогаем —
  // иначе жест мешал бы выделять текст; для мультитача берём только первый палец.
  function bindCalSwipe(root) {
    let pid = null, x0 = 0, y0 = 0, t0 = 0, ax = 0, horiz = false;
    root.addEventListener("pointerdown", (e) => {
      if (e.pointerType === "mouse" || !e.isPrimary) return;
      if (!e.target.closest || !e.target.closest(".cal-swipe")) return;
      pid = e.pointerId; x0 = e.clientX; y0 = e.clientY; t0 = Date.now(); ax = 0; horiz = false;
    }, { passive: true });
    root.addEventListener("pointermove", (e) => {
      if (pid === null || e.pointerId !== pid) return;
      ax = e.clientX - x0;
      if (!horiz && Math.abs(ax) > 12 && Math.abs(ax) > Math.abs(e.clientY - y0)) horiz = true;
    }, { passive: true });
    root.addEventListener("pointerup", (e) => {
      if (pid === null || e.pointerId !== pid) return;
      pid = null;
      if (!horiz) return;
      const far = Math.abs(ax) >= 60;
      const flick = Math.abs(ax) >= 24 && Math.abs(ax) / Math.max(1, Date.now() - t0) >= 0.11;
      if (!far && !flick) return;
      state.calSwipeAt = Date.now();  // тормозим «фантомный» клик по дню после свайпа
      haptic("select");
      shiftCal(ax < 0 ? 1 : -1);
    }, { passive: true });
    root.addEventListener("pointercancel", () => { pid = null; horiz = false; }, { passive: true });
  }
  ...
  function boot() {
    ...
    // P5: маска у края скроллера чипов — живёт вместе с прокруткой/ресайзом/перерисовкой
    document.addEventListener("scroll", queueChipFade, true);
    window.addEventListener("resize", queueChipFade);
    if (window.MutationObserver) {
      new MutationObserver(queueChipFade).observe(document.body, { childList: true, subtree: true });
    }
